...
subs = ['sub14']  # Extend this list as needed
...
# === MANUAL STEP: Enter the 'good' components based on inspection ===
# Example: good_components = [0, 3, 7, 14, 21]
good_components = [4,5,6,7,8,9,10,12]  # <<== Fill this manually after inspecting the plots

# === Initialize list to hold class-wise arrays ===
stimulus_order = [10, 11, 12, 13]
class_wise_data = []  # List of 4 elements, each [n_components, n_trials, n_timepoints]

for stim in stimulus_order:
    # Get trials for this class
    stim_trials = ica_sources[labels == stim]  # shape: [n_trials, n_components, n_timepoints]

    # Select only the good components
    stim_trials_good = stim_trials[:, good_components, :]  # [n_trials, selected_components, n_timepoints]

    # Rearrange to [n_components, n_trials, n_timepoints]
    stim_trials_good = np.transpose(stim_trials_good, (1, 0, 2))

    # Append to list
    class_wise_data.append(stim_trials_good)

# === Optionally save as npz archive ===
save_path = os.path.join(base_path, sub, f"{sub}_ica_epochs_by_class.npz")
np.savez_compressed(save_path, 
    left=class_wise_data[0], 
    right=class_wise_data[1], 
    up=class_wise_data[2], 
    down=class_wise_data[3],
    good_components=np.array(good_components)
)
print(f"Saved ICA epoch data by class to:\n{save_path}")
